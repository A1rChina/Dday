...
export function ReceiptResource() {
  ...
  return (
    <AppPage>
      <AppPageHeader title="型材入库" description="新建和管理型材入库单" />
      <div style={{ marginBottom: 16 }}>
        <AppDrawerForm
          title="新建入库单"
          trigger={<Button type="primary" icon={<PlusOutlined />}>确认入库</Button>}
          drawerProps={{ width: 600, className: 'mes-entity-drawer' }}
          useMutationProps={{
            mutationOptions: {
          ...
          <div className="mes-drawer-content">
            <div className="mes-drawer-card">
              ...
              <div className="mes-drawer-form-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
                <Form.Item label="入库日期" name="receivedDate" rules={[{ required: true }]}><Input type="date" /></Form.Item>
                <Form.Item label="经办人" name="actor" rules={[{ required: true }]}><Input /></Form.Item>
                <Form.Item label="来源类型" name="sourceType" initialValue="manual"><Input disabled /></Form.Item>
                <Form.Item label="备注" name="notes" className="mes-drawer-form-item-full"><Input.TextArea /></Form.Item>
              </div>
            </div>

            <div className="mes-drawer-card">
              <div className="mes-drawer-card-title">入库明细</div>
                    ...
                    {fields.map(({ key, name, ...restField }) => (
                      <div key={key} style={{ border: '1px solid #f0f0f0', padding: 12, borderRadius: 8, background: '#fafafa' }}>
                        <div className="mes-drawer-form-grid">
                          ...
                          <Form.Item {...restField} name={[name, 'warehouseId']} label="仓库ID" rules={[{ required: true }]} initialValue="MAIN"><Input /></Form.Item>
                          <Form.Item {...restField} name={[name, 'locationId']} label="库位ID"><Input /></Form.Item>
                          <Form.Item {...restField} name={[name, 'quantity']} label="数量" rules={[{ required: true }]}><InputNumber min={1} style={{ width: '100%' }} /></Form.Item>
                        </div>
                        {fields.length > 1 && (
      ...
      <AppProTable
        {...tableProps}
        columns={[
          { title: '单据号', dataIndex: 'code', render: (v) => <Typography.Text copyable strong>{v}</Typography.Text> },
          { title: '入库日期', dataIndex: 'receivedDate' },
